...
{include file="$GUI_THEME_PATH/inc-private/auth_js.tpl"}
...
<h3 class="color-pink">{l s='Shopping cart summary'}</h3>
...
<p class="clear"><label for="passwd">{l s='Password'}</label> <span><input
...
<p class="clear">{if isset($back)}<input type="hidden" class="hidden"
	...
	<span class="btn"><span><input type="submit" id="SubmitLogin"
	 name="SubmitLogin" class="button" 	value="{l s='Log in'}" /></span></span></p>
<p class="lost_password right"><a href="{$base_dir}password.php">{l s='Forgot
your password?'}</a></p>
