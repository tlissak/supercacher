{if $products}
	...
	{foreach from=$products item=product name=i}
		...
		<dd class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}" style="font-style:italic;margin:0 0 0 10px;">
			<span class="btn"><span><a href="{$link->getProductLink($product.id_product, $product.link_rewrite)}" title="{l s='Product detail'}">{$product.attributes_small|escape:'htmlall':'UTF-8'}</a></span></span>
		</dd>
		{/if}
	{/foreach}
	</dl>
