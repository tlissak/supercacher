<?php /* Smarty version 2.6.20, created on 2010-06-18 12:57:24
         compiled from ../../../GUI/authentication.asp */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'l', '../../../GUI/authentication.asp', 1, false),array('modifier', 'escape', '../../../GUI/authentication.asp', 65, false),array('modifier', 'stripslashes', '../../../GUI/authentication.asp', 65, false),array('modifier', 'intval', '../../../GUI/authentication.asp', 245, false),)), $this); ?>
<?php ob_start(); ?><?php echo smartyTranslate(array('s' => 'Login'), $this);?>
<?php $this->_smarty_vars['capture']['path'] = ob_get_contents(); ob_end_clean(); ?>
<?php $this->assign('current_step', 'login'); ?> 

<!-- #include file="inc-private/auth_js.tpl" -->
<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => ($this->_tpl_vars['GUI_THEME_PATH'])."/inc-private/auth_js.tpl", 'smarty_include_vars' => array()));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?>



<!-- #include file="inc-global/init.php" -->

<!-- #include file="css/style.css" -->
<!-- #include file="css/global.css" -->

<!-- #include file="js/jquery.search.js" -->
<!-- #include file="js/init.js" -->
<!-- #include file="inc-global/header.html" -->



<!-- #include file="inc-global/base_dir_js.html" -->
<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => ($this->_tpl_vars['GUI_THEME_PATH'])."/inc-global/base_dir_js.html", 'smarty_include_vars' => array()));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?>

<div class="p-20">

<!-- #include file="inc-global/breadcrumb.html" -->
<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => ($this->_tpl_vars['GUI_THEME_PATH'])."/inc-global/breadcrumb.html", 'smarty_include_vars' => array()));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?>

<div class="clear spacer"></div>




<div class="left w-1-5">

<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => ($this->_tpl_vars['GUI_THEME_PATH'])."/inc-private/order-steps.tpl", 'smarty_include_vars' => array()));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?> 
<!-- #include file="inc-private/order-steps.tpl" -->
</div>



<div class="right w-4-5">
<h3 class="color-pink"><?php echo smartyTranslate(array('s' => 'Shopping cart summary'), $this);?>
</h3>

<?php $_smarty_tpl_vars = $this->_tpl_vars;
$this->_smarty_include(array('smarty_include_tpl_file' => ($this->_tpl_vars['GUI_THEME_PATH'])."/inc-global/errors.html", 'smarty_include_vars' => array()));
$this->_tpl_vars = $_smarty_tpl_vars;
unset($_smarty_tpl_vars);
 ?> 
<!-- #include file="inc-global/errors.html" -->


    
<?php if (isset ( $this->_tpl_vars['confirmation'] )): ?>
<div class="left  m-20">
	<p class="success"><?php echo smartyTranslate(array('s' => 'Your account has been successfully created.'), $this);?>
</p>
	<p><span class="btn"><span><a href="<?php echo $this->_tpl_vars['base_dir_ssl']; ?>
my-account.php"><?php echo smartyTranslate(array('s' => 'Your account'), $this);?>
</a></span></span></p>
</div>
<?php else: ?>
<?php if (! isset ( $this->_tpl_vars['email_create'] )): ?>

<div class="left  m-20"><form action="<?php echo $this->_tpl_vars['base_dir_ssl']; ?>
authentication.php" method="post"
	id="create-account_form" class="std">

<h3><?php echo smartyTranslate(array('s' => 'Create your account'), $this);?>
</h3>
<h4><?php echo smartyTranslate(array('s' => 'Enter your e-mail address to create your account'), $this);?>
.</h4>
<p class="text">
<label for="email_create"><?php echo smartyTranslate(array('s' => 'E-mail address'), $this);?>
</label>
<span><input type="text" id="email_create" name="email_create"
	value="<?php if (isset ( $_POST['email_create'] )): ?><?php echo ((is_array($_tmp=((is_array($_tmp=$_POST['email_create'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall') : smarty_modifier_escape($_tmp, 'htmlall')))) ? $this->_run_mod_handler('stripslashes', true, $_tmp) : stripslashes($_tmp)); ?>
<?php endif; ?>"
	class="ipt" /></span></p>
<p class="submit"><?php if (isset ( $this->_tpl_vars['back'] )): ?>
	<input type="hidden" name="back" value="<?php echo ((is_array($_tmp=$this->_tpl_vars['back'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
" /><?php endif; ?> 
	<span class="btn"><span><input
	type="submit" id="SubmitCreate" name="SubmitCreate" value="<?php echo smartyTranslate(array('s' => 'Create your account'), $this);?>
" /></span>
	</span>
	<input	type="hidden"  name="SubmitCreate" 
	value="<?php echo smartyTranslate(array('s' => 'Create your account'), $this);?>
" /></p>

</form>
</div>

<div class="left m-20">
<form action="<?php echo $this->_tpl_vars['base_dir_ssl']; ?>
authentication.php" method="post"
	id="login_form" class="std">

<h3><?php echo smartyTranslate(array('s' => 'Already registered ?'), $this);?>
</h3>
<p class="clear">
	<label for="email"><?php echo smartyTranslate(array('s' => 'E-mail address'), $this);?>
</label> <span><input
	type="text" id="email" name="email"
	value="<?php if (isset ( $_POST['email'] )): ?><?php echo ((is_array($_tmp=((is_array($_tmp=$_POST['email'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall') : smarty_modifier_escape($_tmp, 'htmlall')))) ? $this->_run_mod_handler('stripslashes', true, $_tmp) : stripslashes($_tmp)); ?>
<?php endif; ?>"
	class="ipt right" /></span></p>
<p class="clear"><label for="passwd"><?php echo smartyTranslate(array('s' => 'Password'), $this);?>
</label> <span><input
	type="password" id="passwd" name="passwd"
	value="<?php if (isset ( $_POST['passwd'] )): ?><?php echo ((is_array($_tmp=((is_array($_tmp=$_POST['passwd'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall') : smarty_modifier_escape($_tmp, 'htmlall')))) ? $this->_run_mod_handler('stripslashes', true, $_tmp) : stripslashes($_tmp)); ?>
<?php endif; ?>"
	class="ipt right" /></span></p>
<p class="clear"><?php if (isset ( $this->_tpl_vars['back'] )): ?><input type="hidden" class="hidden"
	name="back" value="<?php echo ((is_array($_tmp=$this->_tpl_vars['back'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
" /><?php endif; ?>
	<span class="btn"><span><input type="submit" id="SubmitLogin"
	 name="SubmitLogin" class="button" 	value="<?php echo smartyTranslate(array('s' => 'Log in'), $this);?>
" /></span></span></p>
<p class="lost_password right"><a href="<?php echo $this->_tpl_vars['base_dir']; ?>
password.php"><?php echo smartyTranslate(array('s' => 'Forgot
your password?'), $this);?>
</a></p>

</form></div>




<div class="clear"></div>
<?php else: ?>
<form action="<?php echo $this->_tpl_vars['base_dir_ssl']; ?>
authentication.php" method="post"
	id="account-creation_form" class="form-label-s">
	<div class="left m-10">
<h3><?php echo smartyTranslate(array('s' => 'Your personal information'), $this);?>
</h3>
<p class="radio required">

	<span><?php echo smartyTranslate(array('s' => 'Title'), $this);?>
</span> 
	
<p class="clear">
	<label for="id_gender1" class="top"><?php echo smartyTranslate(array('s' => 'Mr.'), $this);?>
</label>
	<input type="radio"	name="id_gender" id="id_gender1" value="1" <?php if (isset ( $_POST['id_gender'] ) && $_POST['id_gender'] == 1): ?>checked="checked" <?php endif; ?> /> 
</p>
<p class="clear">	
	<label for="id_gender2" class="top"><?php echo smartyTranslate(array('s' => 'Ms.'), $this);?>
</label>
	<input type="radio" name="id_gender" id="id_gender2"
	value="2" <?php if (isset ( $_POST['id_gender'] ) && $_POST['id_gender'] == 2): ?>checked="checked" <?php endif; ?> /> 
</p>
<p class="clear">	

	<label for="customer_firstname"><?php echo smartyTranslate(array('s' => 'First name'), $this);?>
 <sup>*</sup></label>
	<input onKeyUp="$('#firstname').val(this.value);"
	type="text" class="ipt" id="customer_firstname"
	name="customer_firstname"
	value="<?php if (isset ( $_POST['customer_firstname'] )): ?><?php echo $_POST['customer_firstname']; ?>
<?php endif; ?>" />
</p>

<p class="clear">
	<label for="customer_lastname"><?php echo smartyTranslate(array('s' => 'Last name'), $this);?>
 <sup>*</sup></label>
	<input onKeyUp="$('#lastname').val(this.value);"
	type="text" class="ipt" id="customer_lastname"
	name="customer_lastname"
	value="<?php if (isset ( $_POST['customer_lastname'] )): ?><?php echo $_POST['customer_lastname']; ?>
<?php endif; ?>" />
</p>
<p class="clear"><label for="email"><?php echo smartyTranslate(array('s' => 'E-mail'), $this);?>
 <sup>*</sup></label>
	<input
	type="text" class="ipt" id="email" name="email"
	value="<?php if (isset ( $_POST['email'] )): ?><?php echo $_POST['email']; ?>
<?php endif; ?>" /> 
</p>
<p class="clear">
	<label for="passwd"><?php echo smartyTranslate(array('s' => 'Password'), $this);?>
 <sup>*</sup></label>
<input type="password" class="ipt" name="passwd" id="passwd" />
<br />
<span class="form_info"><?php echo smartyTranslate(array('s' => '(5 characters min.)'), $this);?>
</span></p>

<p class="clear">
	<span><?php echo smartyTranslate(array('s' => 'Birthday'), $this);?>
</span><br />
 <select id="days" name="days" class="ipt">
	<option value="">-</option>
	<?php $_from = $this->_tpl_vars['days']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['day']):
?>
	<option value="<?php echo ((is_array($_tmp=$this->_tpl_vars['day'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
" <?php if (( $this->_tpl_vars['sl_day'] == $this->_tpl_vars['day'] )): ?> selected="selected"<?php endif; ?>><?php echo ((is_array($_tmp=$this->_tpl_vars['day'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
&nbsp;&nbsp;</option>
	<?php endforeach; endif; unset($_from); ?>
</select> 	<select	id="months" name="months" class="ipt">
	<option value="">-</option>
	<?php $_from = $this->_tpl_vars['months']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['k'] => $this->_tpl_vars['month']):
?>
	<option value="<?php echo ((is_array($_tmp=$this->_tpl_vars['k'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
" <?php if (( $this->_tpl_vars['sl_month'] == $this->_tpl_vars['k'] )): ?> selected="selected"<?php endif; ?>><?php echo smartyTranslate(array('s' => ($this->_tpl_vars['month'])), $this);?>
&nbsp;</option>
	<?php endforeach; endif; unset($_from); ?>
</select>
	<select id="years" name="years" class="ipt">
	<option value="">-</option>
	<?php $_from = $this->_tpl_vars['years']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['year']):
?>
	<option value="<?php echo ((is_array($_tmp=$this->_tpl_vars['year'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
" <?php if (( $this->_tpl_vars['sl_year'] == $this->_tpl_vars['year'] )): ?> selected="selected"<?php endif; ?>><?php echo ((is_array($_tmp=$this->_tpl_vars['year'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
&nbsp;&nbsp;</option>
	<?php endforeach; endif; unset($_from); ?>
</select></p>

<p class="clear">
	<?php echo smartyTranslate(array('s' => 'Sign up for our newsletter'), $this);?>

	<input type="checkbox" class="right" name="newsletter"
	id="newsletter" value="1" <?php if (isset ( $_POST['newsletter'] ) && $_POST['newsletter'] == 1): ?> checked="checked" <?php endif; ?> /> </p>
<p class="clear">
	<?php echo smartyTranslate(array('s' => 'Receive special offers from our partners'), $this);?>

	<input type="checkbox" name="optin" class="right" id="optin"
		value="1" <?php if (isset ( $_POST['optin'] ) && $_POST['optin'] == 1): ?> checked="checked" <?php endif; ?> /> </p>
	</div>
	
	
<div class="right m-10">
<h3><?php echo smartyTranslate(array('s' => 'Your address'), $this);?>
</h3>
<p class="clear" ><label for="company"><?php echo smartyTranslate(array('s' => 'Company'), $this);?>
</label>
	<input
	type="text" class="ipt" id="company" name="company"
	value="<?php if (isset ( $_POST['company'] )): ?><?php echo $_POST['company']; ?>
<?php endif; ?>" />
</p>
<p class="clear">
	<label for="firstname"><?php echo smartyTranslate(array('s' => 'First name'), $this);?>
 <sup>*</sup></label>
<input type="text" class="ipt" id="firstname" name="firstname"
	value="<?php if (isset ( $_POST['firstname'] )): ?><?php echo $_POST['firstname']; ?>
<?php endif; ?>" />
</p>
<p class="clear"><label for="lastname"><?php echo smartyTranslate(array('s' => 'Last name'), $this);?>
 <sup>*</sup></label>
<input type="text" class="ipt" id="lastname" name="lastname"
	value="<?php if (isset ( $_POST['lastname'] )): ?><?php echo $_POST['lastname']; ?>
<?php endif; ?>" />
</p>
<p class="clear"><label for="address1"><?php echo smartyTranslate(array('s' => 'Address'), $this);?>
 <sup>*</sup></label> <input
	type="text" class="ipt" name="address1" id="address1"
	value="<?php if (isset ( $_POST['address1'] )): ?><?php echo $_POST['address1']; ?>
<?php endif; ?>" />
</p>
<p><label for="address2"><?php echo smartyTranslate(array('s' => 'Address (2)'), $this);?>
</label> <input
	type="text" class="ipt" name="address2" id="address2"
	value="<?php if (isset ( $_POST['address2'] )): ?><?php echo $_POST['address2']; ?>
<?php endif; ?>" />
</p>
<p class="clear">
<label for="postcode"><?php echo smartyTranslate(array('s' => 'Postal code / Zip code'), $this);?>
 <sup>*</sup></label>
<input type="text" class="ipt" name="postcode"
	id="postcode"
	value="<?php if (isset ( $_POST['postcode'] )): ?><?php echo $_POST['postcode']; ?>
<?php endif; ?>" />
</p>
<p class="clear">
	<label for="city"><?php echo smartyTranslate(array('s' => 'City'), $this);?>
 <sup>*</sup></label>
		<input	type="text" class="ipt" name="city" id="city"
	value="<?php if (isset ( $_POST['city'] )): ?><?php echo $_POST['city']; ?>
<?php endif; ?>" /> 
</p>
<p class="clear">
	<label for="id_country"><?php echo smartyTranslate(array('s' => 'Country'), $this);?>
 <sup>*</sup></label>
	<br />
	<select name="id_country" id="id_country" class="ipt" style="width:160px;"
	onchange="$('.ext_state').hide(); $('#country_'+this.value).show();" >
	<option value="">-</option>
	<?php $_from = $this->_tpl_vars['countries']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['v']):
?>
	<option value="<?php echo $this->_tpl_vars['v']['id_country']; ?>
" <?php if (( $this->_tpl_vars['sl_country'] == $this->_tpl_vars['v']['id_country'] )): ?> selected="selected"<?php endif; ?>><?php echo ((is_array($_tmp=$this->_tpl_vars['v']['name'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
</option>
	<?php endforeach; endif; unset($_from); ?>
</select> </p>

	
	
<!-- id_state -->
<?php $_from = $this->_tpl_vars['countries']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['country']):
?>
	<?php if (isset ( $this->_tpl_vars['country']['states'] )): ?>
		<p class="clear ext_state" id="country_<?php echo ((is_array($_tmp=$this->_tpl_vars['country']['id_country'])) ? $this->_run_mod_handler('intval', true, $_tmp) : intval($_tmp)); ?>
" style="display:none" >
		
		<label for="id_state"><?php echo smartyTranslate(array('s' => 'State'), $this);?>
 <sup>*</sup></label>
		
		<select name="id_state" id="id_state" class="ipt" >
		<?php $_from = $this->_tpl_vars['country']['states']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }$this->_foreach['states'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['states']['total'] > 0):
    foreach ($_from as $this->_tpl_vars['state']):
        $this->_foreach['states']['iteration']++;
?>
			<option value="<?php echo ((is_array($_tmp=$this->_tpl_vars['state']['id_state'])) ? $this->_run_mod_handler('intval', true, $_tmp) : intval($_tmp)); ?>
">		
			<?php echo ((is_array($_tmp=$this->_tpl_vars['state']['name'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>

			</option>			
		<?php endforeach; endif; unset($_from); ?>
		</select>
		</p>
	<?php endif; ?>
<?php endforeach; endif; unset($_from); ?>






</p>
<p class="clear">
	<label for="other"><?php echo smartyTranslate(array('s' => 'Additional information'), $this);?>
</label>

	<textarea name="other" class="ipt" id="other" cols="26" rows="3"><?php if (isset ( $_POST['other'] )): ?><?php echo $_POST['other']; ?>
<?php endif; ?></textarea>
</p>
<p class="clear" ><label for="phone"><?php echo smartyTranslate(array('s' => 'Home phone'), $this);?>
</label>
	<input
	type="text" class="ipt" name="phone" id="phone"
	value="<?php if (isset ( $_POST['phone'] )): ?><?php echo $_POST['phone']; ?>
<?php endif; ?>" /></p>
<p class="clear" ><label for="phone_mobile"><?php echo smartyTranslate(array('s' => 'Mobile phone'), $this);?>
</label> <input
	type="text" class="ipt" name="phone_mobile" id="phone_mobile"
	value="<?php if (isset ( $_POST['phone_mobile'] )): ?><?php echo $_POST['phone_mobile']; ?>
<?php endif; ?>" />
</p>
<p class="clear" id="address_alias"><label for="alias"><sup>*</sup> <?php echo smartyTranslate(array('s' => 'Assign an address title for future reference'), $this);?>
 !</label> 
	<br />
	<input
	type="text" class="ipt" name="alias" id="alias"
	value="<?php if (isset ( $_POST['alias'] )): ?><?php echo $_POST['alias']; ?>
<?php else: ?><?php echo smartyTranslate(array('s' => 'My address'), $this);?>
<?php endif; ?>" />
</p>

	</div>
<?php echo $this->_tpl_vars['HOOK_CREATE_ACCOUNT_FORM']; ?>

<p class="cart_navigation required submit clear-left">

<sup>*</sup><?php echo smartyTranslate(array('s' => 'Required field'), $this);?>

<div class="spacer"></div>

<input type="hidden"
	name="email_create" value="1" /> <?php if (isset ( $this->_tpl_vars['back'] )): ?><input type="hidden"
	class="hidden" name="back" value="<?php echo ((is_array($_tmp=$this->_tpl_vars['back'])) ? $this->_run_mod_handler('escape', true, $_tmp, 'htmlall', 'UTF-8') : smarty_modifier_escape($_tmp, 'htmlall', 'UTF-8')); ?>
" /><?php endif; ?>
<span class="btn"><span><input type="submit" name="submitAccount" id="submitAccount"
	value="<?php echo smartyTranslate(array('s' => 'Register'), $this);?>
" class="exclusive" /></span></span>
	
	
	
</p>

</form>
<?php endif; ?> 
<?php endif; ?></div> <!-- / right w-4-5 -->

<div class="clear"></div>
</div><!--p-20 -->