<?php
class NewsletterHome extends Module{
	private $hookName = 'home' ;
	
 	public function __construct() 	{
 	 	$this->name = 'newsletterhome';
 	 	$this->tab = 'tlissak modules' ;
	 	parent::__construct();
		global $cookie ;
		global $smarty ;
		$this->id_lang = intval($cookie->id_lang) ;
 	 	$this->displayName = $this->l('Newsletter block at homepage');
		...
		$this->_files = array(
			...
			'ext' => array(	0 => 'html',	1 => 'txt'	)
		);
 	}
	public function installNewHook(){
		global $smarty ;
		if ($this->installNewHookDB($this->hookName) ){
			return true ;
		}else{
			return false ;
		}
	...
	private function installNewHookDB(){
		return Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'hook` (`name` ,`title` ,`description` ,`position`)
			VALUES (\''.$this->hookName.'\',\''.$this->hookName. '\',\''.$this->hookName.'\', 1);' ) ;
	}
	private function unistallNewHook(){
		return Db::getInstance()->Execute('DELETE  FROM `'._DB_PREFIX_.'hook` WHERE `name`=\''.$this->hookName.'\' AND `title`=\''.$this->hookName.'\' ;' ) ;
	}
 	public function install(){
		//if (! $this->installNewHook()){			return false ; 		}
 	 	if (parent::install() == false OR $this->registerHook($this->hookName) == false OR $this->installDB() == false)
 	 		return false; 
		return true;
 	}
 	...
 	public function uninstall()	{
		//if (! $this->unistallNewHook()){			return false ; 		}
 	 	if (!parent::uninstall())
 	 		return false;
	...
	private function sendVoucher($email){
		...
		if ($discount = Configuration::get('NW_VOUCHER_CODE'))
			return Mail::send($this->id_lang, 'newsletter_voucher', $this->l('Newsletter voucher'), array('{discount}' => $discount), $email, NULL, NULL, NULL, NULL, NULL, dirname(__FILE__).'/mails/');
		return false;
	}
