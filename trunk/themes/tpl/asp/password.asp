...
<p class="success">{l s='Your password has been successfully reset and
has been sent to your e-mail address:'}
{$email|escape:'htmlall':'UTF-8'}</p>
{else}
...
<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post"
	class="std">
<fieldset>
<p class="text"><label for="email">{l s='Type your e-mail address:'}</label>
...
<p class="submit"><input type="submit" class="button"
	value="{l s='Retrieve'}" /></p>
</fieldset>
</form>
{/if}
<p class="clear"><a href="{$base_dir_ssl}authentication.php"
	title="{l s='Back to Login'}"><img src="{$img_dir}icon/my-account.gif"
	alt="{l s='Back to Login'}" class="icon" /></a><a
	href="{$base_dir}authentication.php" title="{l s='Back to Login'}">{l
s='Back to Login'}</a></p>
